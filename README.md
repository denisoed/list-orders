...
- `/orders/[id]` — страница деталей заказа с кнопкой «Отметить выполненным», ведущей на форму отправки на проверку. Кнопка
  недоступна, если заказ уже находится в финальном статусе.
- `/orders/[id]/submit-review` — новая страница для отправки результата на проверку. Пользователь добавляет минимум одно фото и
  комментарий, после чего данные отправляются в `POST /api/orders/{id}/submit-review`.
- Валидация формы дублирует проверки на сервере: до 6 изображений (JPG/PNG/WEBP, по 10 МБ), комментарий от 10 до 1000 символов.
- Эндпоинт сервера логирует успешные отправки с деталями вложений и возвращает статус `submitted_for_review`.
