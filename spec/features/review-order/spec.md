...
**Specification:** На странице деталей заказа нужно изменить поведение кнопки «Отметить выполненным», чтобы пользователь переходил на новую страницу для отправки заказа на проверку. На новой странице он может прикрепить фотографии выполненной работы, оставить комментарий и отправить задачу на ревью. Интерфейс должен быть минималистичным: только шапка с кнопкой «Назад», названием задачи и форма для вложений и комментария. Длинные названия файлов необходимо сокращать с добавлением троеточия.

## User Stories
- Как исполнитель заказа, я нажимаю «Отметить выполненным» на странице деталей и автоматически перехожу на экран отправки на проверку, чтобы завершить работу без поиска нужного раздела.
- Как исполнитель заказа, я прикрепляю несколько фотографий результата и вижу, что длинные названия файлов корректно обрезаются с троеточием, чтобы интерфейс оставался аккуратным.
- Как исполнитель заказа, я добавляю текстовый комментарий к выполненной работе и отправляю заказ на проверку, чтобы заказчик получил контекст и подтверждение выполнения.

## Main scenarios and rules
- При нажатии кнопки «Отметить выполненным» на экране деталей заказа выполняется навигация на новую страницу подтверждения (например, `/orders/{id}/review`).
- Шапка новой страницы копирует стиль списка заказов: кнопка «Назад» возвращает к предыдущему экрану, рядом отображается название задачи.
...
- Должна быть кнопка отправки, которая сохраняет вложения и комментарий и переводит заказ в состояние «ожидает проверки».
...
- Ожидается, что данные о заказе (название, статус) доступны через существующие API/стор.
