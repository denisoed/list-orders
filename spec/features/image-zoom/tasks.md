...
- [ ] Добавить обработчик клика на миниатюры изображений в `pages/projects/[id]/tasks/new.vue` (секция "Фото примеров") для перехода на страницу просмотра с query-параметрами `projectId` и `source=task-create`.
...
- [ ] Добавить логику получения URL изображения в зависимости от источника:
  - Если `source === 'order-details'` и указан `orderId` — получение из структуры данных заказа (mock данные из `data/orders.ts`).
  - Если `source === 'task-create'` — получение из локального состояния формы (attachments), URL может быть blob URL из File объекта.
...
- [ ] Обработать поддержку blob URL для локально загруженных изображений на странице создания заказа.
...
- [ ] Страница просмотра изображения корректно открывается при клике на миниатюру в разделе "Примеры" на странице деталей заказа.
- [ ] Страница просмотра изображения корректно открывается при клике на миниатюру в разделе "Фото примеров" на странице создания заказа.
- [ ] Поддерживается просмотр как серверных изображений (из заказов), так и локальных blob URL (из формы создания заказа).
