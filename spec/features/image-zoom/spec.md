...
1. Пользователь нажимает на миниатюру изображения в разделе "Фото примеров" на странице создания заказа (`pages/projects/[id]/tasks/new.vue`).
...
- При переходе на страницу через прямую ссылку, если нет предыдущей страницы в истории:
  ...
  - Если открыто со страницы создания заказа — возврат на страницу создания заказа (`/projects/[id]/tasks/new`).
...
9. Страница просмотра изображения должна быть доступна из двух источников: страницы деталей заказа (`pages/orders/[id].vue`) и страницы создания заказа (`pages/projects/[id]/tasks/new.vue`).
