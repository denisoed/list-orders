...
**Plan:** Build a project-scoped "Create Task" page reached from the tasks list, implement form UI and validation that mirrors the provided mockup, integrate with task data sources, and ensure smooth navigation back to the tasks list.

## Data sources / schemas

- Reuse existing project/task composables; extend them with a `createTask` method that accepts `{ projectId, title, description, attachments, link, assigneeId, deadline }`.
- Define a temporary local schema/interface for new task payload if backend contract is pending; ensure mapping to API once available.
...
- Expose a new navigation route `/projects/[id]/tasks/new` in Nuxt pages; ensure router pushes handle `projectId` parameter.
...
- Interface with `useProjectTasks` or a new composable method to refresh the task list after successful creation.

## Architecture / Components

- Add a new page component `pages/projects/[id]/tasks/new.vue` implementing the layout (header, form sections, footer submit).
...
- Use existing Tailwind config, fonts, and Material Symbols setup; ensure `useHead` in new page mirrors tasks page for consistency.
...
- Backend contract for task creation might be undefined, risking rework; mitigate by encapsulating API in composable with clear TODOs.
